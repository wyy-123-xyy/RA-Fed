...
seed = args.seed#seed必须是int，可以自行设置
...
7 net_53                                     X               X
    ...
    setting_arrays = [
        # # [13, 14, 14, 14, 14, 15, 15, 15, 15, 15],   #gammma  1     tau 200
        #[13, 13, 14, 14, 15, 15, 15, 15, 15, 15]    #gammma  2     tau 200
        [13, 13, 14, 14, 14, 14, 14, 14, 15, 15]    #gammma  2     tau 200
        # [14, 13, 13, 13, 13, 15, 15, 15, 15, 15],   #gammma  1     tau 200
        # [14, 14, 13, 13, 13, 13, 15, 15, 15, 15]    #gammma  2     tau 200
        # [15, 15, 12, 12, 13, 13, 12, 12, 14, 14],   #gamma  2    tau 4 
        # [12, 12, 15, 15, 14, 14, 15, 15, 13, 13],
        # [13, 13, 14, 14, 12, 12, 14, 14, 15, 15],
        # [14, 14, 13, 13, 15, 15, 13, 13, 12, 12]            
        # [14, 14, 14, 14, 12, 13, 15, 15, 15, 15],    #gamma   1    tau 4
        # [13, 13, 13, 13, 15, 14, 12, 12, 12, 12],
        # [12, 12, 12, 12, 13, 15, 14, 14, 14, 14],
        # [15, 15, 15, 15, 14, 12, 13, 13, 13, 13]
    ]
    gamma=2
    tau=200
    train_data=[]
    ...
    for i in range(0,1):
        ...
        setting="our2_"+str(args.epochs)+"_"+str(alpha)+'_'+str(args.lr)
        #setting=str(args.epochs)+"_"+str(alpha)+'_'+str(args.lr)+'_'+str(gamma)+'_'+str(tau)+"version3"
        pic_name = './result/ramfed_{}_{}_{}_{}_lep{}_iid{}_{}.pdf'.format(args.dataset, args.model_training_typep, args.model,
                                                                    args.epochs, args.local_ep, args.iid, setting)
        txt_name = './result/ramfed_{}_{}_{}_{}_lep{}_iid{}_{}.txt'.format(args.dataset, args.model_training_typep, args.model,
                                                                    args.epochs, args.local_ep, args.iid, setting)
        npy_name = './result/ramfed_{}_{}_{}_{}_lep{}_iid{}_{}.npy'.format(args.dataset, args.model_training_typep, args.model,
                                                                    args.epochs, args.local_ep, args.iid, setting)
##########################TEMP USE FOR STRATING LOSS
        ...
        start_client_update=[]
        for i in range(0,4):
            start_client_update.append(torch.zeros(starting_weights[mlp_keys[i]].size(), dtype=starting_weights[mlp_keys[i]][0].dtype))
        ...
        for iter in range(args.epochs):
            ...
            #setting_array = setting_arrays[iter%4]
            ...
            for id, idx in enumerate(idxs_users):
                ...
                if model_training_typep=="L":
                    ...
                    sys.exit(0)
                #typep = setting_array[id]
                #args.num_users个client
                #train_loader=DataLoader(train_data, batch_size,drop_last=True,shuffle=True)
                #self.ldr_train = DataLoader(train_data, batch_size=self.args.local_bs, shuffle=True)
                ...
                #local_c = LocalUpdate(args=args, dataset=train_data[idx])
            ...
            w_g = FedAvg3(w_glob,w_locals, type_array, local_w_masks, local_b_masks)
            for i in range (0,4):
    ...
    #print(avg_epoch_train_loss[i],avg_epoch_test_loss[i],avg_epoch_test_acc[i])
